...
public class LvTextAdapter extends BaseLvAdapter<Basic_Dictionary_Kj> {
    ...
    public LvTextAdapter(Context context, List<Basic_Dictionary_Kj> list, String columnValue) {
        super(context, list);
    ...
    public View simpleGetView(final int position, View convertView, ViewGroup parent) {
        ...
        if (selectPosition == -1) {
            viewHolder.tvroomnumber.setTextColor(ContextCompat.getColor(context, columnValue.equals(list.get(position).getCOLUMNVALUE()) ? R.color.bg_blue : R.color.font_9));
        } else {
